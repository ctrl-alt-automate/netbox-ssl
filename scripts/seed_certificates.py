...
tenant_prod, _ = Tenant.objects.get_or_create(slug="production", defaults={"name": "Production"})
tenant_dev, _ = Tenant.objects.get_or_create(slug="development", defaults={"name": "Development"})
...
for data in cas_data:
    ca, created = CertificateAuthority.objects.get_or_create(name=data["name"], defaults=data)
    cas[data["name"]] = ca
...
for spec in certs_spec:
    ...
    existing = Certificate.objects.filter(common_name=spec["cn"], issuer=spec["issuer"]).first()
    if existing:
...
for data in policies:
    policy, created = CompliancePolicy.objects.get_or_create(name=data["name"], defaults=data)
