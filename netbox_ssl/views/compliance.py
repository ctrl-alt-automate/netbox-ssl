...
class ComplianceCheckDeleteView(generic.ObjectDeleteView):
    """Delete a single compliance check result.

    Results are regenerated by the checker, so deleting one is only ever
    housekeeping -- but NetBoxTable's ActionsColumn reverses this view for every
    row, so its absence raised NoReverseMatch and 500'd the whole list.
    """
